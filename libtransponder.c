/*
 * Autotranspond - Autotranspond feature for all the protocols
...
#define PLUGIN_ID			"prpl-autotranspond"
#define PLUGIN_STATIC_NAME	"Autotranspond"
#define PLUGIN_AUTHOR		"Fufu Fang <fangfufu@fangfufu.co.uk>"
...
#define	PREFS_ONLINE		PREFS_PREFIX "/online"
...
typedef struct _PurpleAutoTranspond PurpleAutoTranspond;
typedef struct _AutoTranspondProtocolOptions	AutoTranspondProtocolOptions;

struct _PurpleAutoTranspond
{
	PurpleBuddy *buddy;
	char *reply;
};

struct _AutoTranspondProtocolOptions {
 ...
 */
static const char *
get_autotranspond_message(PurpleBuddy *buddy, PurpleAccount *account)
{
	...
	if ((!reply || !*reply) && buddy)
	{
		...
		reply = purple_blist_node_get_string((PurpleBlistNode*)buddy, "autotranspond");

		if ((!reply || !*reply) && PURPLE_BLIST_NODE_IS_BUDDY((PurpleBlistNode*)buddy))
		{
			/* Anything for the contact, then? */
			reply = purple_blist_node_get_string(((PurpleBlistNode*)buddy)->parent, "autotranspond");
		}
	}
	...
	if (!reply || !*reply)
	{
		...
		reply = purple_account_get_string(account, "autotranspond", NULL);
	}
...
static void
written_msg(PurpleAccount *account, const char *who)
{
	PurpleBuddy *buddy;
    ...
    if (purple_prefs_get_bool(PREFS_ONLINE) && purple_presence_is_online(presence)) {
        trigger = TRUE;
    }
	...
	if (reply)
	{
		...
		last_sent = GPOINTER_TO_INT(purple_conversation_get_data(conv, "autotranspond_lastsent"));
		now = time(NULL);

		/* Have we spent enough time after our last autotranspond? */
		if (now - last_sent >= (purple_prefs_get_int(PREFS_MINTIME)*60))
		{
			count_sent = GPOINTER_TO_INT(purple_conversation_get_data(conv, "autotranspond_count"));
			maxsend = purple_prefs_get_int(PREFS_MAXSEND);

			/* Have we sent the autotranspond enough times? */
			if (count_sent < maxsend || maxsend == -1)
			{
				purple_conversation_set_data(conv, "autotranspond_count", GINT_TO_POINTER(++count_sent));
				purple_conversation_set_data(conv, "autotranspond_lastsent", GINT_TO_POINTER(now));
...
set_auto_reply_cb(PurpleBlistNode *node, char *message)
{
	if (!message || !*message)
		message = " ";
	purple_blist_node_set_string(node, "autotranspond", message);
}
...
set_auto_reply(PurpleBlistNode *node, gpointer plugin)
{
	...
	/* XXX: There should be a way to reset to the default/account-default autotranspond */

	message = g_strdup_printf(_("Set autotranspond message for %s"),
					purple_buddy_get_contact_alias(buddy));
	purple_request_input(plugin, _("Set Autotranspond Message"), message,
					_("The following message will be sent to the buddy when "
						"the buddy sends you a message and autotranspond is enabled."),
					get_autotranspond_message(buddy, account), TRUE, FALSE,
...
context_menu(PurpleBlistNode *node, GList **menu, gpointer plugin)
{
	...
	action = purple_menu_action_new(_("Set _Autotranspond Message"),
...
{
	AutoTranspondProtocolOptions *arpo;
	PurplePluginProtocolInfo *info = PURPLE_PLUGIN_PROTOCOL_INFO(plg);

	arpo = g_new(AutoTranspondProtocolOptions, 1);

	arpo->message = purple_account_option_string_new(_("Autotranspond message"),
													 "autotranspond", NULL);
	arpo->off = purple_account_option_bool_new(_("Turn off autotranspond"),
													 "ar_off", FALSE);
...
{
	PurplePluginProtocolInfo *info = PURPLE_PLUGIN_PROTOCOL_INFO(plg);
	AutoTranspondProtocolOptions *arpo = g_hash_table_lookup(options, plg);
	GList *l = NULL;
...
plugin_load(PurplePlugin *plugin)
{
    ...
    purple_signal_connect(purple_conversations_get_handle(), "buddy-typing", plugin,
...
get_plugin_pref_frame(PurplePlugin *plugin)
{
	...
	pref = purple_plugin_pref_new_with_label(
        _("Send autotranspond messages when"));
	purple_plugin_pref_frame_add(frame, pref);

    pref = purple_plugin_pref_new_with_name_and_label(PREFS_ONLINE,
                    _("When my account is _online"));
    purple_plugin_pref_frame_add(frame, pref);
	...
	pref = purple_plugin_pref_new_with_name_and_label(PREFS_X_INVISIBLE,
					_("Do not autotranspond when invisible."));
	purple_plugin_pref_frame_add(frame, pref);
	...
	pref = purple_plugin_pref_new_with_name_and_label(PREFS_USESTATUS,
						_("Autotranspond with status message"));
	...
	purple_plugin_pref_add_choice(pref, _("Only when there's no autotranspond message"),
...
init_plugin(PurplePlugin *plugin)
{
	...
	info.name = _("Autotranspond");
    info.summary = _("Automatically respond with a message when your contact starts typing");
	info.description = _("This plugin lets you set automatically send a message "
			"when your contact starts typing. You can set the global response message from the "
            "plugin options dialog. To set some specific response message for "
			"a particular buddy, right click on the buddy in the buddy-list "
            "window. To set response messages for some accounts, go to the "
	...
	purple_prefs_add_string(PREFS_PREFIX_MSG, _(""));
